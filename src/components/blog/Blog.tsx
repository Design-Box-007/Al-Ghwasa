...
const componentMap = {
  ...
  Blog6: dynamic(() => import("@/data/Blogs/Blog6")),
...
const Blog = () => {
  ...
  const blogData = blogListData.find(
    (blog: BlogType) => blogName === formatToHyphenated(blog.title)
  );

  if (!blogData) return <>BLOG NOT FOUND</>;

  const BlogComponent =
    componentMap[blogData.component as keyof typeof componentMap];

  if (!BlogComponent) return <>BLOG NOT FOUND</>;

  return (
    <main className="p-[10px] px-4 pt-[150px] font-poppins space-y-10">
