...
import { z } from "zod";
import { DetectionContactForm as ContactFormType } from "@/types";
...
const DetectionContactForm: React.FC<{
  className: string;
  btnClassName: string;
}> = ({ className = "", btnClassName = "" }) => {
  const [loading, setLoading] = useState(false);
  const { selectedItem } = useDetectionData();
  ...
  const {
    ...
    defaultValues: {
      product: selectedItem
        ? {
            chemical: selectedItem.tube_name,
            range: selectedItem.measuring_range,
            tubeModel: selectedItem.tube_model,
            partNo: selectedItem.tube_no,
            QB: selectedItem.no_test_per_box,
            type: selectedItem.tube_type,
          }
        : {
            chemical: "",
            range: "",
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="relative z-0 w-full">
            ...
            {errors.email && (
              <p className="text-yellow-300 text-xs">{errors.email.message}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="relative z-0 w-full">
            ...
            {errors.mobile && (
              <p className="text-yellow-300 text-xs">{errors.mobile.message}</p>
            )}
          </div>
          ...
          <div className="relative z-0 w-full">
            <input
              ...
              value={
                selectedItem
                  ? `${selectedItem.tube_name}-${selectedItem.measuring_range}-${selectedItem.tube_model}-${selectedItem.tube_no}-${selectedItem.no_test_per_box}-${selectedItem.tube_type}`
                  : ""
              }
              readOnly
              className="peer block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-white focus:outline-none focus:ring-0 focus:border-white"
        ...
        <div className="relative z-0 w-full">
          ...
          {errors.message && (
            <p className="text-yellow-300 text-xs">{errors.message.message}</p>
          )}
        </div>
...
export default DetectionContactForm;
